...
export default function BookingForm({
  ...
  const [formData, setFormData] = useState<BookingData>({
    ...
    passengers: 2,
    includeMeals: false,
    mealCount: 2, // ✅ Default matches passengers usually
    includeTickets: false,
    startTime: "06:00 AM",
  });
  ...
  // 3b. LOGIC: SYNC MEAL COUNT WITH PASSENGERS (Optional UX Enhancement)
  // When passengers change, update meal count if user hasn't manually set a weird number?
  // For simplicity, we'll keep them independent or default them together.
  useEffect(() => {
      // Optional: Auto-update meal count to match passengers if meals are not yet selected
  ...
  const totals = useMemo(() => {
    const base = packageDetails?.price || 0;
    // Tickets usually match passengers (unless you want separate ticket count too)
    const tickets = formData.includeTickets ? formData.passengers * currentTicketPrice : 0;
    // ✅ Meals now use specific mealCount
    const meals = formData.includeMeals ? formData.mealCount * currentMealPrice : 0;
    
    return { base, tickets, meals, grandTotal: base + tickets + meals };
  }, [packageDetails, formData.passengers, formData.includeTickets, formData.includeMeals, formData.mealCount, currentTicketPrice, currentMealPrice]);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                {/* ✅ MEAL OPTION WITH COUNTER */}
                <div className={cn("relative overflow-hidden rounded-xl border p-4 transition-all duration-300 backdrop-blur-md group",
                    formData.includeMeals ? "bg-green-500/10 border-green-500 shadow-[0_0_15px_rgba(34,197,94,0.15)]" : "bg-black/40 border-white/10 hover:border-green-500/30 hover:bg-white/5")}>
                    
                    <div className="flex items-start justify-between gap-4 cursor-pointer" onClick={() => setFormData(p => ({ ...p, includeMeals: !p.includeMeals }))}>
                        <div className="flex items-start gap-3">
                            <div className={cn("p-2 rounded-lg transition-colors", formData.includeMeals ? "bg-green-500 text-black" : "bg-white/10 text-neutral-400 group-hover:text-white")}>
                                <Utensils size={18} />
                            </div>
                            <div>
                                <h4 className={cn("text-sm font-bold transition-colors", formData.includeMeals ? "text-green-400" : "text-white")}>Picnic Meals</h4>
                                <p className="text-[10px] text-neutral-400 leading-relaxed mt-1 line-clamp-2">Breakfast/lunch packs included.</p>
                            </div>
                        </div>
                        <div className="text-right shrink-0">
                            <div className="text-white font-mono font-bold">${currentMealPrice}</div>
                            <div className="text-[9px] text-neutral-500 uppercase">Per Meal</div>
                        </div>
                    </div>

                    {/* ✅ COUNTER FOR MEALS (Visible only if enabled) */}
                    {formData.includeMeals && (
                        <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between animate-in slide-in-from-top-2 fade-in duration-300">
                            <span className="text-[10px] text-green-400 font-bold uppercase tracking-wider">Quantity</span>
                            <div className="flex items-center gap-3 bg-black/40 rounded-lg border border-green-500/30 p-1">
                                <button type="button" onClick={() => setFormData(p => ({ ...p, mealCount: Math.max(1, p.mealCount - 1) }))} className="w-6 h-6 flex items-center justify-center bg-white/5 hover:bg-green-500/20 text-white rounded transition-colors"><Minus size={12} /></button>
                                <span className="text-sm font-mono font-bold text-white w-4 text-center">{formData.mealCount}</span>
                                <button type="button" onClick={() => setFormData(p => ({ ...p, mealCount: Math.min(20, p.mealCount + 1) }))} className="w-6 h-6 flex items-center justify-center bg-white/5 hover:bg-green-500/20 text-white rounded transition-colors"><Plus size={12} /></button>
                            </div>
                        </div>
                    )}

                    <div className={cn("absolute top-2 right-2 transition-all duration-300", formData.includeMeals ? "opacity-100 scale-100" : "opacity-0 scale-50")}>
                        <CheckCircle2 size={16} className="text-green-500 fill-green-500/20" />
                    </div>
                </div>

                {/* TICKET OPTION (Simple Toggle for now, matches pax count) */}
                <AddOnCard
                  active={formData.includeTickets}
                  onClick={() => setFormData(p => ({ ...p, includeTickets: !p.includeTickets }))}
                  icon={<Ticket size={18} />}
                  title="Park Tickets"
                  desc="Entrance tickets purchased for you (matches guest count)."
            ...
            <div className="bg-black/60 backdrop-blur-2xl rounded-xl p-6 border border-green-500/20 font-mono text-sm relative overflow-hidden shadow-lg">
              ...
              <div className="space-y-3 mb-4 text-neutral-300 text-xs">
                ...
                {formData.includeMeals && (
                  <PriceRow
                    label={`Meals (${formData.mealCount} x $${currentMealPrice})`}
                    amount={totals.meals}
                    color="text-green-400/80"
                  />
                )}
